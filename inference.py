...
class Processer:
    ...
    def infer(
        ...
        aligned_image, mask_image, align_matrix = self.preprocess(
            image, mask_image, sr_flag)
        ...
        generated_image = postprocess_image_masking(
            generated_image, aligned_image, mask_image_postproc)
        generated_image = np.array(generated_image)
        generated_image = cv2.cvtColor(generated_image, cv2.COLOR_BGR2RGB)

        # upscale output with sr model  to increase quality
        if sr_flag:
            generated_image = self.upsampler.process(
                generated_image, outscale=2)

        generated_image = reverse_alignment(
            generated_image, w, h, reverse_align_matrix)
...
if __name__ == "__main__":
    ...
    for i in range(3):
        generated_image = processer.infer(
            image,
            "A Japanes style living room",
            mask_image=None,
            super_resolution=True,
            step=50)
        cv2.imwrite(
            "outputs/generated_image_" +
            str(i) +
            ".jpg",
            generated_image)
